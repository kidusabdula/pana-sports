// lib/hooks/cms/useStandings.ts
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query'
import { Standing, CreateStanding, UpdateStanding } from '@/lib/schemas/standing'

// API helper functions
async function fetchStandings(params?: { league_id?: string; season?: string }) {
  ...
  const res = await fetch(`/api/standings${queryString ? '?' + queryString : ''}`);
...
// React Query hooks
export function useStandings(params?: { league_id?: string; season?: string }) {
  return useQuery({
