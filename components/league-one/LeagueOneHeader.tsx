// components/league-one/LeagueOneHeader.tsx
"use client";
...
import { Trophy, Calendar, BarChart3, Users, TrendingUp, Clock, ChevronRight, Shield, Star } from 'lucide-react';
...
export default function LeagueOneHeader() {
  const t = useTranslations('leagueOne');
  const [activeTab, setActiveTab] = useState("overview");

  // Sample League One data
  const leagueInfo = {
    name: "League One",
    season: "2025/2026",
    teams: 12,
    ...
    totalMatches: 120,
    status: "In Progress",
    currentRound: "Round of 12",
    defendingChampion: "Bahir Dar Kenema",
        ...
        <Card className="bg-zinc-800/20 backdrop-blur-sm border-zinc-700/30">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-lg font-semibold">{t('roundOf12')}</p>
            </div>
        ...
        <Card className="bg-zinc-800/20 backdrop-blur-sm border-zinc-700/30">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-lg font-semibold">{t('bahirdarKenema')}</p>
            </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="teams" className="mt-6">
          <div className="text-center text-muted-foreground py-8">
            ...
            <p className="text-lg">{t('currentRound')}: {t('roundOf12')}</p>
            <p className="text-lg mb-4">{t('defendingChampion')}: {t('bahirdarKenema')}</p>
          </div>
        </TabsContent>
